...
use bulletin_store::{Ballot, Ciphertext, default_path, load, save};
use chrono::Utc;
use rand::RngCore;
use rand::rngs::OsRng;
...
/// Full submit-ballot flow against the bulletin file at `path`.
///
...
pub fn submit_ballot_impl(path: &Path, args: SubmitArgs) -> Result<SubmitResult, String> {
    let mut rng = OsRng;
    submit_ballot_with_rng(path, args, &mut rng)
}
...
pub fn submit_ballot_with_rng<R: RngCore>(
    ...
    let mut bulletin = load(path).map_err(|e| format!("load bulletin: {e}"))?;
    ...
    save(path, &bulletin).map_err(|e| format!("save bulletin: {e}"))?;
...
/// Public default-path accessor for the CLI binary.
pub fn cli_default_path() -> PathBuf {
    default_path()
}
